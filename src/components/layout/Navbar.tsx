    ...
    return (
        <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95 shadow-md py-3' : 'bg-transparent py-6'} ${isHidden ? '-translate-y-full' : 'translate-y-0'}`}>
            <div className="max-w-10xl mx-auto px-6 md:px-12 flex justify-between items-center">
                        ...
                        >
                            <a
                                ...
                                className={`text-[15px] font-bold transition-all whitespace-nowrap ${activeSection === item.name
                                    ? 'bg-[#FFE4D6] text-inamice-blue px-6 py-2 rounded-full'
                                    : 'text-inamice-blue hover:text-inamice-orange'
                                    }`}
                            >
                                {item.name}
                            </a>
                                        ...
                                        <div className="flex flex-col">
                                            {item.subItems.map((sub, idx) => (
                                                <a
                                                    key={idx}
                                                    href={sub.href}
                                                    className="px-6 py-3 text-inamice-blue hover:text-inamice-orange font-bold text-sm transition-colors rounded-lg hover:bg-slate-50"
                                                >
                                                    {sub.name}
                                                </a>
                                            ))}
                ...
                {/* Social Icons */}
                <div className="hidden md:flex items-center gap-5 text-inamice-blue ml-12">
                    <a href="#" className="hover:text-inamice-orange transition-colors">
                ...
                {/* Mobile Menu Toggle */}
                <button
                    className="md:hidden p-2 text-inamice-blue"
                        ...
                        {NAVIGATION.map((item) => (
                            <div key={item.name} className="flex flex-col gap-2">
                                <a
                                    ...
                                    className={`text-base font-bold p-2 ${item.name === 'About' ? 'text-inamice-orange px-6 py-2 bg-[#FFE4D6] rounded-full w-fit' : 'text-inamice-blue'}`}
                                >
                                    {item.name}
                                </a>
                                {item.subItems && (
                                    <div className="flex flex-col gap-3 pl-6 mt-1">
                                        {item.subItems.map((sub, idx) => (
                                            <a
                                                ...
                                                className="text-inamice-blue/80 font-bold text-sm"
                                            >
                                                {sub.name}
                                            </a>
                                        ))}
